...
BSNS_YEAR        = "2025"
STOCK_BATCH_SIZE = 25   # 한 번에 처리할 종목 수
...
def fetch_period_items(api_key, corp_code, bsns_year):
    """분기별 DART API 호출(총 4회). (period_items, rcept_nos) 반환."""
    url = "https://opendart.fss.or.kr/api/fnlttSinglAcnt.json"
    period_items, rcept_nos = {}, {}
    ...
    for period, code in PERIOD_CODES.items():
        ...
        if data.get("status") == "000":
            ...
            if items:
                rcept_nos[period] = items[0].get("rcept_no")
        else:
            period_items[period] = []
...
def upsert_to_supabase(rows):
    """rows를 DB_BATCH_SIZE씩 나눠 upsert. 실패 시 (status_code, message) 반환."""
...
def save_failures_to_excel(failures, bsns_year):
    """failures: [{"stock_code", "corp_code", "reason"}, ...]"""
...
def main():
    ...
    print(f"   총 {total_stocks}개 종목 → {total_batches}개 배치 (배치당 {STOCK_BATCH_SIZE}개)\n")

    total_inserted = 0

    for batch_num, i in enumerate(range(0, total_stocks, STOCK_BATCH_SIZE), start=1):
        stock_batch = all_stock_codes[i:i + STOCK_BATCH_SIZE]
        print(f"[배치 {batch_num}/{total_batches}] 종목: {stock_batch[0]} ~ {stock_batch[-1]}")
        ...
        for stock_code in stock_batch:
            ...
            if not corp_code:
                failures.append({"stock_code": stock_code, "reason": "corp_codes 매칭 없음 (우선주 등)"})
                continue

            # 2. DART 조회
            try:
                period_items, rcept_nos = fetch_period_items(dart_api_key, corp_code, BSNS_YEAR)
            except Exception as e:
            ...
            # 3. 행 생성
            try:
                rows = build_db_rows(corp_code, stock_code, BSNS_YEAR, period_items, rcept_nos)
        ...
        # 4. 배치 단위 upsert
        if batch_rows:
            errors = upsert_to_supabase(batch_rows)
            if errors:
                for err in errors:
                    print(f"  [Supabase 오류] {err}")
                for stock_code in stock_batch:
                    failures.append({"stock_code": stock_code, "reason": f"Supabase upsert 오류: {errors[0]}"})
            else:
                total_inserted += len(batch_rows)
                print(f"  → {len(batch_rows)}행 적재 완료 (누적 {total_inserted}행)")
