...
def save_failures_to_excel(failures, bsns_year):
    ...
    filename = f"annual_failures_{bsns_year}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    wb.save(filename)
    return filename
...
def main():
    ...
    for batch_num, i in enumerate(range(0, total_stocks, API_BATCH_SIZE), start=1):
        ...
        # 2. DART 연간 멀티 조회 (1회 호출로 배치 내 전 기업 처리)
        try:
        ...
        for stock_code, corp_code in corp_map.items():
            try:
                ...
                if not items:
                    failures.append({"stock_code": stock_code, "corp_code": corp_code, "reason": "연간 데이터 없음"})
                    continue
                row = build_annual_row(
                    corp_code, stock_code, BSNS_YEAR,
                    items, rcept_map.get(corp_code)
                )
